...
class TestPasswordResetRequest:
    ...
    def test_empty_username_rejected(self):
        with pytest.raises(ValidationError):
            PasswordResetRequest(username="", new_password="newpass123", verification_token="token-123")

    def test_empty_verification_token_rejected(self):
        with pytest.raises(ValidationError):
            PasswordResetRequest(username="scott", new_password="newpass123", verification_token="")

    def test_short_new_password_rejected(self):
        with pytest.raises(ValidationError):
            PasswordResetRequest(username="scott", new_password="short", verification_token="token-123")
