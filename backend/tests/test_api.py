...
class TestResetPasswordEndpoint:
    ...
    def test_missing_brand_returns_422(self, client):
        body = {**VALID_RESET_BODY}
        del body["brand"]
        res = client.post(RESET_ENDPOINT, json=body)
        assert res.status_code == 422
    ...
    def test_invalid_brand_returns_422(self, client):
        body = {**VALID_RESET_BODY, "brand": "hertz"}
        res = client.post(RESET_ENDPOINT, json=body)
        assert res.status_code == 422
...
class TestVerifyCredentialsEndpoint:
    def test_success(self, client, mock_oracle_success, mock_create_verification_token):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "avis", "username": "scott", "current_password": "tiger"},
        )
        assert res.status_code == 200
        data = res.json()
        assert data["success"] is True
    ...
    def test_budget_brand_success(self, client, mock_oracle_success, mock_create_verification_token):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "budget", "username": "scott", "current_password": "tiger"},
        )
        assert res.status_code == 200
        data = res.json()
        assert data["success"] is True
    ...
    def test_oracle_error_returns_200_with_failure(self, client, mock_oracle_error):
        with mock_oracle_error(1017):
            res = client.post(
                VERIFY_ENDPOINT,
                json={"brand": "avis", "username": "scott", "current_password": "wrongpw"},
            )
        assert res.status_code == 200
        data = res.json()
        assert data["success"] is False
    ...
    def test_missing_username_returns_422(self, client):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "avis", "current_password": "tiger"},
        )
        assert res.status_code == 422
    ...
    def test_empty_username_returns_422(self, client):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "avis", "username": "", "current_password": "tiger"},
        )
        assert res.status_code == 422
    ...
    def test_missing_current_password_returns_422(self, client):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "avis", "username": "scott"},
        )
        assert res.status_code == 422
    ...
    def test_empty_current_password_returns_422(self, client):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "avis", "username": "scott", "current_password": ""},
        )
        assert res.status_code == 422
    ...
    def test_invalid_brand_returns_422(self, client):
        res = client.post(
            VERIFY_ENDPOINT,
            json={"brand": "hertz", "username": "scott", "current_password": "tiger"},
        )
        assert res.status_code == 422
